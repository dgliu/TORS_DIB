...
class Objective:
    ...
    def __call__(self, trial: Trial) -> float:
        ...
        lam = trial.suggest_categorical('lambda', [1e-4])
        batch_size = trial.suggest_categorical('batch_size', [512, 1024])
        lr = trial.suggest_categorical('learning_rate', [0.005])
        alpha = trial.suggest_uniform('alpha', 0.0005, 0.001)
        gamma = trial.suggest_uniform('gamma', 0.0001, 0.0005)

        np.random.seed(self.seed)
        tf.set_random_seed(self.seed)

        model = DIBRNCF(self.num_users, self.num_items, np.int(rank), np.int(batch_size), lamb=lam, alpha=alpha,
...
class DIBRNCF(object):
    ...
    def _build_graph(self):
        ...
        with tf.variable_scope('dibr-ncf'):
            # Placehoder
            ...
            # Variable to learn
            share_size = np.ceil(self._embed_dim / 3).astype(np.int32)
            independent_size = (self._embed_dim - np.ceil(self._embed_dim / 3)).astype(np.int32)

            z_user_embeddings = tf.Variable(tf.random_normal([self._num_users, independent_size],
                                                             stddev=1 / (independent_size ** 0.5), dtype=tf.float32))
            c_user_embeddings = tf.Variable(tf.random_normal([self._num_users, independent_size],
                                                             stddev=1 / (independent_size ** 0.5), dtype=tf.float32))
            shared_user_embeddings = tf.Variable(tf.random_normal([self._num_users, share_size * 2],
                                                                  stddev=1 / ((share_size * 2) ** 0.5),
                                                                  dtype=tf.float32))
            user_zero_vector_1 = tf.get_variable(
                'user_zero_vector_1', [self._num_users, independent_size],
                initializer=tf.constant_initializer(0.0, dtype=tf.float32), trainable=False)
            user_zero_vector_2 = tf.get_variable(
                'user_zero_vector_2', [self._num_users, share_size * 2],
                initializer=tf.constant_initializer(0.0, dtype=tf.float32), trainable=False)
            user_zero_vector_3 = tf.get_variable(
                'user_zero_vector_3', [self._num_users, independent_size + share_size * 2],
                initializer=tf.constant_initializer(0.0, dtype=tf.float32), trainable=False)

            self.z_user_embeddings = tf.concat([z_user_embeddings, user_zero_vector_3], 1)
            self.c_user_embeddings = tf.concat([user_zero_vector_3, c_user_embeddings], 1)
            self.zs_user_embeddings = tf.concat([z_user_embeddings, shared_user_embeddings, user_zero_vector_1], 1)
            self.cs_user_embeddings = tf.concat([user_zero_vector_1, shared_user_embeddings, c_user_embeddings], 1)
            self.zc_user_embeddings = tf.concat([z_user_embeddings, user_zero_vector_2, c_user_embeddings], 1)

            z_item_embeddings = tf.Variable(tf.random_normal([self._num_items, independent_size],
                                                             stddev=1 / (independent_size ** 0.5), dtype=tf.float32))
            c_item_embeddings = tf.Variable(tf.random_normal([self._num_items, independent_size],
                                                             stddev=1 / (independent_size ** 0.5), dtype=tf.float32))
            shared_item_embeddings = tf.Variable(tf.random_normal([self._num_items, share_size * 2],
                                                                  stddev=1 / ((share_size * 2) ** 0.5),
                                                                  dtype=tf.float32))
            item_zero_vector_1 = tf.get_variable(
                'item_zero_vector_1', [self._num_items, independent_size],
                initializer=tf.constant_initializer(0.0, dtype=tf.float32), trainable=False)
            item_zero_vector_2 = tf.get_variable(
                'item_zero_vector_2', [self._num_items, share_size * 2],
                initializer=tf.constant_initializer(0.0, dtype=tf.float32), trainable=False)
            item_zero_vector_3 = tf.get_variable(
                'item_zero_vector_3', [self._num_items, independent_size + share_size * 2],
                initializer=tf.constant_initializer(0.0, dtype=tf.float32), trainable=False)

            self.z_item_embeddings = tf.concat([z_item_embeddings, item_zero_vector_3], 1)
            self.c_item_embeddings = tf.concat([item_zero_vector_3, c_item_embeddings], 1)
            self.zs_item_embeddings = tf.concat([z_item_embeddings, shared_item_embeddings, item_zero_vector_1], 1)
            self.cs_item_embeddings = tf.concat([item_zero_vector_1, shared_item_embeddings, c_item_embeddings], 1)
            self.zc_item_embeddings = tf.concat([z_item_embeddings, item_zero_vector_2, c_item_embeddings], 1)
            ...
            with tf.variable_scope("ncf_loss"):
                ...
                zs_users = tf.nn.embedding_lookup(self.zs_user_embeddings, self.user_idx)
                zs_items = tf.nn.embedding_lookup(self.zs_item_embeddings, self.item_idx)

                zs = tf.concat([zs_users, zs_items], axis=1)

                zs_encoded = tf.matmul(zs, self.mlp1_weights) + self.mlp1_bias
                zs_x_ij = tf.squeeze(tf.matmul(tf.nn.tanh(zs_encoded), self.mlp2_weights))
                self.sigmoid_zs_x_ij = tf.nn.sigmoid(zs_x_ij)

                cs_users = tf.nn.embedding_lookup(self.cs_user_embeddings, self.user_idx)
                cs_items = tf.nn.embedding_lookup(self.cs_item_embeddings, self.item_idx)

                cs = tf.concat([cs_users, cs_items], axis=1)

                cs_encoded = tf.matmul(cs, self.mlp1_weights) + self.mlp1_bias
                cs_x_ij = tf.squeeze(tf.matmul(tf.nn.tanh(cs_encoded), self.mlp2_weights))
                ...
                mf_loss = tf.reduce_mean(
                    tf.nn.sigmoid_cross_entropy_with_logits(logits=zs_x_ij, labels=self.label) -
                    self._alpha * tf.nn.sigmoid_cross_entropy_with_logits(logits=z_x_ij, labels=self.label) -
                    self._alpha * tf.nn.sigmoid_cross_entropy_with_logits(logits=c_x_ij, labels=self.label) +
                    self._alpha * tf.nn.sigmoid_cross_entropy_with_logits(logits=zc_x_ij, labels=self.label) +
                    self._gamma * tf.nn.sigmoid_cross_entropy_with_logits(logits=cs_x_ij, labels=self.label)
                )
            ...
            with tf.variable_scope('l2_loss'):
                ...
                unique_users = tf.nn.embedding_lookup(self.zs_user_embeddings, unique_user_idx)

                unique_item_idx, _ = tf.unique(self.item_idx)
                unique_items = tf.nn.embedding_lookup(self.zs_item_embeddings, unique_item_idx)
    ...
    def train_model(self, matrix_train, matrix_valid, matrix_test, epoch=100, metric='AUC', topK=50,
        ...
        for i in tqdm(range(epoch)):
            ...
            valid_rating_prediction = self.sess.run(self.sigmoid_zs_x_ij,
            ...
            for step in range(len(all_batches)):
                ...
                vector_predict = self.sess.run([self.sigmoid_zs_x_ij], feed_dict=feed_dict)
            ...
            test_rating_prediction = self.sess.run(self.sigmoid_zs_x_ij,
...
def dibrncf(matrix_train, matrix_valid, matrix_test, embeded_matrix=np.empty(0), iteration=1000, lam=0.01, alpha=0.01,
            rank=200, gamma=0.01, batch_size=500, learning_rate=1e-3, optimizer="Adam", seed=0, gpu_on=False,
            metric='AUC', topK=50, is_topK=False, searcher='optuna', n_trials=20, **unused):
    progress = WorkSplitter()

    progress.section("DIBR-NCF: Set the random seed")
    np.random.seed(seed)
    tf.set_random_seed(seed)

    progress.section("DIBR-NCF: Training")
    ...
    if searcher == 'grid':
        model = DIBRNCF(m, n, rank, batch_size, lamb=lam, alpha=alpha, gamma=gamma, learning_rate=learning_rate,
