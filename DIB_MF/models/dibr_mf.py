...
class Objective:
    ...
    def __call__(self, trial: Trial) -> float:
        ...
        lam = trial.suggest_categorical('lambda', [1e-5])
        batch_size = trial.suggest_categorical('batch_size', [8192])
        lr = trial.suggest_categorical('learning_rate', [0.001, 0.005, 0.01])
        alpha = trial.suggest_uniform('alpha', 0.01, 0.015)
        gamma = trial.suggest_uniform('gamma', 0.0005, 0.001)

        np.random.seed(self.seed)
        tf.set_random_seed(self.seed)

        model = DIBRMF(self.num_users, self.num_items, np.int(rank), np.int(batch_size), lamb=lam, alpha=alpha,
...
class DIBRMF(object):
    ...
    def _build_graph(self):
        ...
        with tf.variable_scope('dibr-mf'):
            # Placehoder
            ...
            with tf.variable_scope("mf_loss"):
                ...
                zs_x_ij = tf.reduce_sum(tf.multiply(zs_users, zs_items), axis=1)
                ...
                cs_x_ij = tf.reduce_sum(tf.multiply(cs_users, cs_items), axis=1)
    ...
    def train_model(self, matrix_train, _matrix_train, matrix_valid, matrix_test, epoch=100, metric='AUC', topK=50,
        ...
        for i in tqdm(range(epoch)):
            ...
            RQ, Y = self.sess.run([self.zs_user_embeddings, self.zs_item_embeddings])
...
def dibrmf(matrix_train, matrix_valid, matrix_test, embeded_matrix=np.empty(0), iteration=400, lam=0.01, alpha=0.01,
    ...
    progress.section("DIBR-MF: Set the random seed")
    np.random.seed(seed)
    tf.set_random_seed(seed)

    progress.section("DIBR-MF: Training")
    ...
    if searcher == 'grid':
        model = DIBRMF(m, n, rank, batch_size, lamb=lam, alpha=alpha, gamma=gamma, learning_rate=learning_rate,
